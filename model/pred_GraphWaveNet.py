...
def trainModel(name, mode, XS, YS):
    ...
    for epoch in range(EPOCH):
        ...
        loss_sum, n = 0.0, 0
        model.train()
        train_iter = tqdm(train_iter, ncols=100)
    ...
    YS, YS_pred = scaler.inverse_transform(np.squeeze(YS).reshape(-1, CHANNEL)), scaler.inverse_transform(
        np.squeeze(YS_pred).reshape(-1, CHANNEL))
    print('YS.shape, YS_pred.shape,', YS.shape, YS_pred.shape)
...
def testModel(name, mode, XS, YS):
    ...
    print('YS.shape, YS_pred.shape,', YS.shape, YS_pred.shape)
    YS, YS_pred = scaler.inverse_transform(np.squeeze(YS).reshape(-1, CHANNEL)).reshape(-1, TIMESTEP_OUT, N_NODE,
                                                                                        CHANNEL), scaler.inverse_transform(
        np.squeeze(YS_pred).reshape(-1, CHANNEL)).reshape(-1, TIMESTEP_OUT, N_NODE, CHANNEL)
    print('YS.shape, YS_pred.shape,', YS.shape, YS_pred.shape)
...
########################################################### 
GPU = sys.argv[-1] if len(sys.argv) == 2 else CUDA
device = torch.device("cuda:{}".format(GPU)) if torch.cuda.is_available() else torch.device("cpu")
###########################################################
data = np.load(FLOWPATH)['data'][:, :, 0]
scaler = StandardScaler()
data = (scaler.fit_transform(data.reshape(-1, CHANNEL))).reshape(data.shape[0], data.shape[1], CHANNEL)
